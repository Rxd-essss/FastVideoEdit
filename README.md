...
- **[stable-diffusion.cpp](https://github.com/leejet/stable-diffusion.cpp)** —
  MIT, опциональный движок локальной генерации контекстных картинок для
  авто-обогащения (полностью оффлайн, без torch). Бинарь + DLL в комплект не
  входят: положите `sd-cli.exe` (и соседние `stable-diffusion.dll`,
  `cudart64_12.dll`, `cublas*64_12.dll`) в `tools/`. Модель — **SDXL-Turbo**
  Q4_0 GGUF (~3.94 ГБ, лицензия Stability AI Community License) — скачивается
  пользователем отдельно; путь указывается в `render.imagegen.imagegen_model`.
  Без бинаря/модели генерация деградирует на эмодзи-фолбэк (задача не падает),
  движок строго opt-in (`render.imagegen.imagegen_enabled`).

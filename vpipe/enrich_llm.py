...
def match_user_assets(points: list, folder, llm, log: LogFn = _noop,
                      *, emoji_map_path: Optional[Path] = None) -> int:
    """§4 Tier 1: проставить ассеты точкам-иллюстрациям (мутирует ``points``).
    ...
    folder — path-traversal отбит). Не нашли файл — эмодзи-фолбэк по
    emoji_map.json (``asset_kind="emoji"`` + ``emoji``); совсем нет — точка
    остаётся ``asset_kind="none"``.

    Безопасен к сбоям: нет папки / пустой индекс / упавший LLM -> эмодзи-фолбэк
    (или none), исключение наружу НЕ летит (сбойный этап не валит пасс). Точки —
    сырые dict-кандидаты иллюстраций (как из ``_detect_illustrations``: с
    ``payload.concept``). Возврат — число точек, которым проставлен ассет
    (user|emoji)."""
    ...
    for i, p in enumerate(ill):
        ...
        if name:                                 # Tier 0 fallback: эмодзи
            ...
            pl["asset_path"] = ""
            matched += 1
        # иначе остаётся asset_kind="none" (предложение без ассета)
    return matched
...
def detect_all(transcript: Optional[Transcript], cutlist: Optional[CutList],
    ...
    - этап assets (§4 Tier 0/1): если ``params.image_source`` ∈ {user_folder,
      auto} И ``user_folder`` задан — ПОСЛЕ детектора иллюстраций один LLM-вызов
      сопоставляет точки с папкой юзера (match_user_assets); иначе/без папки —
      эмодзи-фолбэк по emoji_map.json (тоже без сети). Не нашли — none;
    ...
    # Этап assets (§4 Tier 0/1, вес 10%): иллюстрации идут от детектора с
    # asset_kind="none" — здесь проставляем ассет. image_source=emoji: сразу
    # эмодзи-фолбэк (folder=None -> match_user_assets уходит в emoji_map);
    # auto/user_folder: LLM-матчинг папки юзера, не нашли -> эмодзи -> none.
    if run_assets:
        try:

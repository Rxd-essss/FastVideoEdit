...
_CTA_SYSTEM = (
    ...
    "строка \"\".\n"
    "- type=\"comment\" — 1-2 места для призыва написать в комментарии. "
    "comment_question — КОНКРЕТНЫЙ короткий вопрос зрителю, который вытекает "
    "из того, о чём говорится именно в этом месте видео. Вопрос живой, "
    "разговорный, как задал бы сам автор. Не общий («что думаете?»), а по "
    "теме момента.\n\n"
    ...
    "здесь. Не больше 5 элементов всего. Если автор сам в этом месте уже "
    "просит лайк/комментарий — это хорошее место, можно использовать."
)
...
def _detect_cta(eff: _EffStream, transcript: Transcript, llm, ka_next,
    ...
    for r in (raw if isinstance(raw, list) else []):
        ...
        if typ == "comment" and not question:
            continue                               # comment без вопроса -> дроп

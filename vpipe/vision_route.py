...
# --- строгость роутера (измерено на 48 реальных моментах юзера) ------------------
# Один и тот же кадр+реплика дают ВЕТО 75% при строгой формулировке и 19% при
# мягкой; решения совпадают лишь в 40%. Значит порог — не техническая истина, а
# РЕДАКТОРСКИЙ ВКУС, и выбирать его должен человек, а не зашитая строка.
#   strict — «оставляем только то, что точно уместно» (12 предложений из 48);
#   soft   — «доверяем монтажёру, режем лишь явный слоп» (39 из 48).
SYSTEM = (
...
SYSTEM_SOFT = (
    "Ты — арт-директор монтажа YouTube-ролика. Тебе дают несколько подряд идущих "
    "кадров одного момента видео, реплику спикера и ВИЗУАЛ, который авто-монтажёр "
    "хочет наложить поверх кадра. По умолчанию ДОВЕРЯЙ монтажёру: врезка полезна, "
    "пока не доказано обратное.\n"
    "Ставь insert=false ТОЛЬКО в двух явных случаях:\n"
    "1. на кадре крупный план говорящего человека И визуал никак не связан с речью;\n"
    "2. экран уже показывает РОВНО ТО ЖЕ САМОЕ (врезка дословно дублирует видимое) "
    "или закроет собой важное содержимое экрана.\n"
    "Во всех остальных случаях insert=true и подходящий kind (diagram / image / "
    "list_card). Сомневаешься — оставляй insert=true.\n"
    "Если выбрал kind=list_card — ОБЯЗАТЕЛЬНО заполни поле card: title и items "
    "(2-4 коротких пункта) строго по смыслу кадра и реплики.\n"
    "Отвечай СТРОГО одним JSON-объектом по схеме."
)

STRICTNESS = {"strict": SYSTEM, "soft": SYSTEM_SOFT}


def system_prompt(strictness: str) -> str:
    """Промпт по выбранной строгости; незнакомое значение → строгий (дефолт)."""
    return STRICTNESS.get(str(strictness or "").lower(), SYSTEM)
...
def route(items: list, video, ffmpeg_bin: str, client, cfg,
    ...
    for idx, it in enumerate(targets):
        ...
        try:
            v = client.chat_json(system_prompt(getattr(cfg, "strictness",
                                                       "strict")),
                                 user, VERDICT_SCHEMA,

...
class VisionRouteCfg(_Base):
    ...
    # Строгость — РЕДАКТОРСКИЙ выбор, а не техническая истина: на 48 реальных
    # моментах одна лишь формулировка промпта меняет вето с 75% («strict»,
    # выживает 12 предложений) до 19% («soft», выживает 39), совпадение решений
    # всего 40%. Поэтому порог отдан пользователю. strict — дефолт: жалоба была
    # именно на переизбыток неуместных врезок.
    strictness: str = "strict"              # strict | soft
